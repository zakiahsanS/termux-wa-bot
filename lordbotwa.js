...
//Kasi credit gw SangLord

//dan juga Subscribe MikazuMZ jgn cuma ganti nama bot
...
const { color, bgcolor } = require('./lordgans/color')
const { animesaran } = require('./sanglord/animesaran')
const { animesaran2 } = require('./sanglord/animesaran2')
const { help } = require('./lordgans/help')
const { rules } = require('./sanglord/rules')
const { listsurah } = require('./sanglord/listsurah')
const { donasi } = require('./lordgans/donasi')
const { fetchJson } = require('./lordgans/fetcher')
const { recognize } = require('./lordgans/ocr')
const { exec } = require("child_process")
const { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./lordgans/functions')
const tiktod = require('tiktok-scraper')
const brainly = require('brainly-scraper')
const ffmpeg = require('fluent-ffmpeg')
const cd = 4.32e+7
const { removeBackgroundFromImageFile } = require('remove.bg')
const { ind } = require('./zaki')
const apivhtear = 'dapapikeybotwa'
...
const anyt = JSON.parse(fs.readFileSync('./database/json/antiyt.json'))
...
const client = new WAConnection()
   ...
   console.log(color('(+)','white'),color('ZakiGans','red'),color('(+)','white'),color('SQAN CODENYA!!','blue'),color('YOU','white'),color('TUBE','red'),color('MikazuMz','yellow'))
})
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const isAntyt = isGroup ? antiyt.includes(from) : false
...
// ANTI LINK GRUP
		        ...
		        setTimeout( () => {
			        client.groupRemove(from, [kic]).catch((e)=>{reply(`*LordBOT~ HARUS JADI ADMINâ—*`)})
		        }, 3000)
		        setTimeout( () => {
			        client.updatePresence(from, Presence.composing)
		...
		if (!isGroup && isCmd) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(command), 'from', color(sender.split('@')[0]), 'args :', color(args.length))
			...
			if (antiyt && isGroup && isBotGroupAdmins){
            if (args.match(/(https:\/\/youtube.com)/gi)) {
                const check = await mek.inviteInfo(Farhan);
                if (!check) {
                    return
                } else {
                    reply('*[GROUP LINKYT DETECTOR!]*\nKamu mengirimkan link youtube, maaf kamu segera di kick dari grup.').then(() => {
                        client.groupRemove()
                    })
                }
            }
        ...
        if (isGroup && isBadWord) {
			...
			switch(command) {
				...
				if (!isRegistered) return reply(ind.noregis())
						reply(ind.wait())
						fs.writeFileSync('./sanglord/anime.json', JSON.stringify(anime))
					break
				case 'animesaran':
					if (!isRegistered) return reply(ind.noregis())
			...
			case 'audio':
				client.updatePresence(from, Presence.composing) 
 				if (!isRegistered) return reply(ind.noregis())
				 ayam = fs.readFileSync('./sanglord/music.js');
			...
			case 'kdu':
				client.updatePresence(from, Presence.composing) 
 			if (!isRegistered) return reply(ind.noregis())
				 data = fs.readFileSync('./sanglord/husbu.js');
			...
			case 'wallpaperty':
					...
					reply(ind.wait())
					paper = body.slice(7)
					u = await fetchJson(`http://lolhuman.herokuapp.com/api/wallpaper/querty=${paper}?apikey=f629447917e6324e21f82064`, {method: 'get'})
			...
			case 'amv':
			 ...
			 reply(`*Sedang di Prosess...*`)
				if (!isRegistered) return reply(ind.noregis())
				 data = fs.readFileSync('./sanglord/amv.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
			...
			case 'animesaran2':
				...
				case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('ngapain afk tod?')
                                        if (!isRegistered) return reply(ind.noregis())
...
//CASE BADWORD
		...
		case 'nsfwblowjob':
					...
					if (!isRegistered) return reply(ind.noregis())
						...
						client.sendMessage(from, buffervv, image, {quoted: mek, caption: 'Coli teroooooooossss'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
	...
	if (!isRegistered) return reply(ind.noregis())
           ...
           client.sendMessage(from, buffer, image, {quoted: mek, caption: '*follow ig @sanglord_*'})
		await limitAdd(sender)
           break
		case 'waifu':
                        if (!isRegistered) return reply(ind.noregis())
		...
		case 'dadu':
			...
			break
				...
				client.updatePresence(from, Presence.composing) 
				 if (!isRegistered) return reply(ind.noregis())
                        if (isLimit(sender)) return reply(ind.limitend(pusname))
				reply(ind.wait())
				 data = fs.readFileSync('./sanglord/drak.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 darkjokes = await getBuffer(randKey.result)
                 client.sendMessage(from, darkjokes, image, {quoted: mek, caption: '\`\`\`follow ig @sanglord_\`\`\`'})
				await limitAdd(sender)
				break
			case 'asupan':
				client.updatePresence(from, Presence.composing) 
				if (!isRegistered) return reply(ind.noregis())
				 data = fs.readFileSync('./sanglord/asupan.js');
		                 jsonData = JSON.parse(data);
                		 randIndex = Math.floor(Math.random() * jsonData.length);
		                 randKey = jsonData[randIndex];
		                buffer = await getBuffer(randKey.result)
                ...
                if(isNaN(jumblah)) return await reply('jumplah kok huruf? gapernah sekolah?')
			...
			case 'kurumi':
				if (!isRegistered) return reply(ind.noregis())
					...
					client.sendMessage(from, nye, image, { caption: 'follow ig @sanglord_', quoted: mek })
					await limitAdd(sender) 
					break 
			case 'miku':
				if (!isRegistered) return reply(ind.noregis())
					...
					nye = await getBuffer(ku)
					client.sendMessage(from, nye, image, { caption: '*follow ig @sanglord_â™¥*', quoted: mek })
					await limitAdd(sender) 
					break 
			case 'naruto':
					if (!isRegistered) return reply(ind.noregis())
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					client.sendMessage(from, nye, image, { caption: '*pake doang donasi kgk tomlol*', quoted: mek })
					await limitAdd(sender)
					break 
				case 'sasuke':
				if (!isRegistered) return reply(ind.noregis())
			...
			if (args.length < 1) return reply(ind.wrongf())
					...
					client.sendMessage(from, vintage, image, {caption: 'nih anjing ${vin}', quoted: mek})
					await limitAdd(sender)
					break
                 case 'avengers':
                 if (!isRegistered) return reply(ind.noregis())
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   const aruga1 = q.substring(0, q.indexOf('|') - 0)
					...
					client.sendMessage(from, aruga, image, {caption: 'Nih su', quoted: mek})
					await limitAdd(sender)
					break 
					case 'summer':
					if (!isRegistered) return reply(ind.noregis())
					...
					client.sendMessage(from, arugazzz, image, {caption: 'Nih su', quoted: mek})
					await limitAdd(sender)
					break
					case 'lava':
					if (!isRegistered) return reply(ind.noregis())
					...
					client.sendMessage(from, shizuka, image, {caption: 'Nih su', quoted: mek})
					await limitAdd(sender)
					break
					case 'blackpink':
					if (!isRegistered) return reply(ind.noregis())
					...
					client.sendMessage(from, shizukaa, image, {caption: 'Nih su', quoted: mek})
					await limitAdd(sender)
					break
					case 'randomanime':
					if (!isRegistered) return reply(ind.noregis())
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (args.length < 1) return reply(ind.wrongf())
					silk = body.slice(10)
					if (silk.length > 7) return reply('Teksnya kepanjangan Tolol, maksimal 6 karakter')
					...
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
              	   			 if (args.length < 1) return reply('teksnya mana njg?')
                    			hm = `${body.slice(8)}`
                    			text1 = hm.split("/")[0];
                    			text2 = hm.split("/")[1];                    
...
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}hartatahta dappa`)
har = body.slice(12)
buffer = await getBuffer(`https://api.vhtear.com/hartatahta?text=${har}&apikey=${apivhtear}` )
reply('「❗」Follow Ig @sanglord_')
client.sendMessage(from, buffer, image, {quoted: mek})
await limitAdd(sender)
break
case 'indo1':
...
if (isLimit(sender)) return reply(ind.limitend(pusname))
		...
		break
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
			   ...
			   case 'antiyt':
					...
					if (args[0] == 'on') {
						if (isSimi) return reply('Mode antiyt sudah aktif')
						anyt.push(from)
						fs.writeFileSync('./src/antiyt.json', JSON.stringify(anyt))
						reply(`Sukses mengaktifkan mode antiyt`)
					} else if (args[0] == 'off') {
						anyt.splice(from, 1)
						fs.writeFileSync('./src/antiyt.json', JSON.stringify(anyt))
						reply('Sukes menonaktifkan mode antiyt')
					} else {
				...
				if (!isGroup) return reply(ind.groupo()) 
					...
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=bokep&apikey=dapapikeybotwa`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isNsfw) return reply(ind.nsfwoff())
				 data = fs.readFileSync('./sanglord/18.js');
				...
				res = await fetchJson(`https://api.vhtear.com/randomloli&apikey=${apivhtear}`, {method: 'get'})
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'nih om'})
				} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
				}
				...
				case 'ero':
				    try {
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					client.sendMessage(from, buf, image, {quoted: mek,caption: "*HAYO NGAPAIN PEGANG TYTYD?*"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('*ERROR*')
			...
			case 'nsfwtrap':
					...
					if (!isRegistered) return reply(ind.noregis())
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '*HAYO NGAPAIN PEGANG TYTYD?*'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('*ERROR*')
					}
			...
			case 'waifu2':
				...
				if (!isNsfw) return reply(ind.nsfwoff())
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ciee ngaceng'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
			...
			case 'hentai':
				...
				if (!isRegistered) return reply(ind.noregis())
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hadehh'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
		...
		case 'resepmasakan':  
			...
			if (!isGroupAdmins) return reply(ind.admin())
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group LordBOT~ :\n\n\`\`\``
					no = 0
		...
		case 'daftar':
                ...
                if(isNaN(umurUser)) return await reply('*Umur harus angka goblooog!*')       
	...
	.then(() => {client.sendMessage(from, options, text)})
		...
		case 'makequote':
                ...
                var gh = body.slice(12)
					...
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {caption: 'Subs DappaUhuy', quoted: mek})
					await limitAdd(sender)
					break
                    case 'truth':
                if (!isRegistered) return reply(ind.noregis())
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname)) 
			...
			break
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					reply(ind.wait)
					bufasfer = await getBuffer(`https://api.vhtear.com/galaxytext?text=${teks}&apikey=dapapikeybotwa`, {method: 'get'})
					client.sendMessage(from, bufasfer, image, {quoted: mek})
					break
				case 'lordgroup':
					if (!isRegistered) return reply(ind.noregis())
				    if (isLimit(sender)) return reply(ind.limitend(pusname))
					client.sendMessage(from, '*LORD GROUP*\n\nLink : _BELUM TERSEDIA_ ', text, { quoted: mek })
					break
				case 'stiker': 
				case 'sticker':
				case 's':
			    ...
			    if (args.length < 1) return client.sendMessage(from, 'Kode bahasanya mana om?', text, {quoted: mek})
					const gtts = require('./lordgans/gtts')(args[0])
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh kontak lordgans <•_•> ,Jangan Lupa Save Ya (*>*)',MessageType.text, { quoted: mek} )
		const DAPPAGANTENG = fs.readFileSync('./dappauhuy/sanglord');
                client.sendMessage(from, DAPPAGANTENG, MessageType.image, {quoted: mek, caption: '*INI OWNER GW ZAKI GANTENG:) KAN😎*'})
                 break
		break    
           	case 'setname':
                if (!isGroup) return reply(ind.groupo())
		...
		if (budy.includes(`Dappa`)) {
                const Dappa = fs.readFileSync('./dappauhuy/Dappa');
...
else if (text == 'TEST')
{
  client.sendMessage(id, tambahan.test, MessageType.text);
		...
		if (budy.includes(`bot`)) {
                  reply(`Iya LordBOT~ disini ketik #help ya kak`)
		...
		if (budy.includes(`Bot`)) {
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  ...
                  reply(`*${pushname}*, Command *${prefix}${command}* Tidak Ada Di Dalam *${prefix}menu LordBOT~!*`)
